...
@section('js_after')
    ...
    <script>
        ...
        $('body').on("click",".btn-edit",function(){
            var id = $(this).attr("id");
            $('#form_result').html('');
            window.location.href ="/updateinvestigasi/"+id+"/edit";
        });
        ...
        $("#notApprove").on("submit",function(e){
            e.preventDefault()
            var id = $('#id').text();
            var dt = $(this).serialize();
            $.ajax({
        ...
        //status send document client
        $('body').on("click",".btn-sent-document",function(){
            var id = $('#id').text();
            $('#form_result').html('');
            $.ajax({
                url: "/sendApprove/"+id,
                ...
                dataType : "json",
                success:function(html){
                    $("#modal-sent-document").modal("show")
                    $("#id").val(html.data.id)
                }
            });
        });

        $("#getsentdocument").on("submit",function(e){
            e.preventDefault()
            var id = $('#id').text();
            var dt = $(this).serialize();
            $.ajax({
                ...
                data: $(this).serialize(),
                success:function(){
                    $("#modal-sent-document").modal("hide");
                    One.helpers('jq-notify', 
                        {type: 'success', icon: 'fa fa-check me-1', message: 'Sent success!'});
                    location.reload();
                }
            })
        })
    ...
    <script type="text/javascript">
        ...
        $('body').on("click",".btn-edit-kesimpulan",function(){
            var id = $(this).attr("id");
            window.location.href ="/editkesimpulan/"+id+"";
        });
        ...
        $('.js-dataTable-rekomendasi').dataTable({
            ...
            columns: [
                {data: 'DT_RowIndex' , name: 'id', width: '1%'},
                {
                        ...
                        {
                            if (row.baku == "0"){
                                return row.rekomendasi+' '+'<span class="fw-semibold d-inline-block py-1 px-3 rounded-pill bg-success-light text-success fs-sm"></span>';
                            }
                            if (row.baku == "1"){
                                return row.rekomendasi+' '+'<span class="fw-semibold d-inline-block py-1 px-3 rounded-pill bg-success-light text-success fs-sm">Baku</span>';
                            }
        ...
        $("#createrekomndasibaku").on("submit",function(e){
            e.preventDefault()
            var data = $(this).serialize();
            $.ajax({
                url: "{{url('addrekomendasi')}}",
        ...
        $(".btn-print-worksheet").on("click",function(){
            var id = $('#id').text();
            window.open("{{ route('printWorksheet',$detail->id)}}",'_blank');
        })


        $('body').on("click",".btn-rekbaku",function(){
...
@section('content')
            ...
            <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center py-2">
                <div class="flex-grow-1">
                    ...
                    <h2 class="fs-base lh-base fw-medium text-muted mb-0">
                        No Case : {{$detail->no_case}}{{$detail->number_case}}| Status : 
                        @if ($detail->status =='0')
                            <span class="fw-semibold d-inline-block py-1 px-3 rounded-pill bg-warning-light text-warning fs-sm">In Progress</span>
                        
                        @endif
                        @if ($detail->status =='1' and ($detail->status_sent_client =='0'))
                            <span class="fw-semibold d-inline-block py-1 px-3 rounded-pill bg-success-light text-success fs-sm">Completed</span>
                        @endif
                        @if ($detail->status =='1' and ($detail->status_sent_client =='1'))
                            <span class="fw-semibold d-inline-block py-1 px-3 rounded-pill bg-success-light text-success fs-sm">Completed</span> <span class="fw-semibold d-inline-block py-1 px-3 rounded-pill bg-success-light text-success fs-sm">sent</span>
                        @endif
                        @if ($detail->status =='2')
    ...
    <div class="content">
        ...
        <div class="block block-rounded">
            <div class="block-header block-header-default">
                ...
                <button type="submit"  class="btn btn-alt-primary me-1 btn-sm btn-print-worksheet"><i class="fa fa-print text-info me-1"></i>Print Worksheet</button>
                        ...
                        <table class="table table-bordered table-striped table-vcenter">
                            ...
                            <tbody style="line-height: 10px;">
                                ...
                                <tr>
                                 ...
                                 <tr>
                                    <td class="fw fs-sm">Tambahan Waktu</td>
                                    <td class="fw fs-sm">{{$detail->tambahan_waktu}}</td>
                                </tr>
                            </tbody>
                        </table>
        ...
        <div class="block block-rounded">
            <div class="block-header block-header-default">
                ...
                <a href="{{ route('getlampiran',$detail->id)}}" class="btn btn-alt-success me-1 btn-sm">
                    <i class="fa fa-plus text-info me-1"></i>Lampiran</a>  
                <a href="{{ route('updateinvestigasi.show',$detail->id)}}" class="btn btn-alt-success me-1 btn-sm">
                    <i class="fa fa-plus text-info me-1"></i>Update Investigasi</a>  
                @endif            
            </div>
        ...
        <div class="block block-rounded">
            <div class="block-header block-header-default">
                ...
                @if ($detail->status=='0')
                <a href="{{ route('createkesimpulan',$detail->id)}}" class="btn btn-alt-success me-1 btn-sm">
                    <i class="fa fa-plus text-info me-1"></i>Add Kesimpulan</a>              
                @endif
            </div>
        ...
        <div class="block block-rounded">
            <div class="block-header block-header-default">
                ...
                @if ($detail->status=='0')
                <button class="btn btn-alt-success me-1 btn-sm btn-rekbaku">
                    <i class="fa fa-plus text-info me-1"></i>Rekomendasi Baku</button>
        ...
        <div class="row push">
            <div class="col-lg-12 text-center">
                ...
                @if ($detail->status =='0' or $detail->status =='2')
                    <button type="submit"  class="btn btn-alt-primary me-1 btn-sm btn-cetak-sementara"><i class="fa fa-print text-info me-1"></i>Generate Report Sementara</button>
                @endif
                ...
                @endif
                @if (($detail->status =='2' or $detail->status =='0') and ($user->role =='master' or $user->role =='spv'))
                    <a class="btn btn-alt-primary me-1 btn-sm btn-approve"><i class="fa fa-check text-success me-1"></i>Approve</a>
                @endif
                @if (($detail->status =='2') and ($user->role =='master' or $user->role =='spv'))
                    <a class="btn btn-alt-warning me-1 btn-sm btn-not-approve"><i class="fa fa-check text-warning me-1"></i>Not Approve</a>
                @endif
                @if ($detail->status =='1' and ($user->role =='master' or $user->role =='spv'))
                ...
                @endif
                @if ($detail->status =='1' and ($detail->status_sent_client == '0') and ($user->role =='master' or $user->role =='spv'))
                <a class="btn btn-alt-success me-1 btn-sm btn-sent-document"><i class="fa fa-check text-warning me-1"></i>Sent Document</a>
                @endif
            </div>             
        </div>
        ...
        <div class="modal-dialog modal-vcenter" role="document">
            <div class="modal-content">
                ...
                <div class="block block-rounded block-transparent mb-0">
                    ...
                    <div class="block-content block-content-full text-end bg-body">
                        <button type="button" class="btn btn-sm btn-alt-secondary me-1" data-bs-dismiss="modal">Cancel</button>
                        <button type="submit" class="btn btn-sm btn-danger btn-getsend-approve">Approve</button>
                    </div>
                </form>
                </div>
            </div>
    ...
    <!-- modal not approve-->
    <div class="modal fade" id="modal-not-approve" tabindex="-1" role="dialog" aria-labelledby="modal-block-fadein" aria-hidden="true">
        <div class="modal-dialog modal-vcenter" role="document">
            <div class="modal-content">
                <span id="form_result"></span>
                <form id="notApprove">
                <div class="block block-rounded block-transparent mb-0">
                    <div class="block-header block-header-default">
                    <h3 class="block-title">Not Approve Investigasi</h3>
                    <input type="text" value="0" name="status" id="status" hidden>
                    <div class="block-options">
                        <button type="button" class="btn-block-option" data-bs-dismiss="modal" aria-label="Close">
                        <i class="fa fa-fw fa-times"></i>
                    ...
                    <div class="block-content fs-sm text-center">
                        <h5>Hasil Investigasi tidak di approve?</h5>
                    </div>
                    <div class="block-content block-content-full text-end bg-body">
                        <button type="button" class="btn btn-sm btn-alt-secondary me-1" data-bs-dismiss="modal">Cancel</button>
                        <button type="submit" class="btn btn-sm btn-danger btn-not-approve">Not Appove</button>
                    </div>
                </form>
                </div>
            </div>
        ...
        <div class="modal-dialog modal-vcenter" role="document">
            <div class="modal-content">
                ...
                <div class="block block-rounded block-transparent mb-0">
                    ...
                    <input type="text" value="{{$user->id}}" name="user_id_approve" id="user_id_approve" hidden>
                    <div class="block-options">
                        <button type="button" class="btn-block-option" data-bs-dismiss="modal" aria-label="Close">
                        <i class="fa fa-fw fa-times"></i>
                        </button>
                    ...
                    <div class="block-content block-content-full text-end bg-body">
                        <button type="button" class="btn btn-sm btn-alt-secondary me-1" data-bs-dismiss="modal">Cancel</button>
                        <button type="submit" class="btn btn-sm btn-danger btn-getcancel-send">Approve</button>
                    </div>
                </form>
                </div>
            </div>
        ...
        <div class="modal-dialog modal-vcenter" role="document">
            <div class="modal-content">
                ...
                <div class="block block-rounded block-transparent mb-0">
                    ...
                    <div class="block-content block-content-full text-end bg-body">
                        <button type="button" class="btn btn-sm btn-alt-secondary me-1" data-bs-dismiss="modal">Cancel</button>
                        <button type="submit" class="btn btn-sm btn-danger btn-cancel-approve">Cancel Approve</button>
                    </div>
                </form>
                </div>
            </div>
    ...
    <!-- modal send dokument-->
    <div class="modal fade" id="modal-sent-document" tabindex="-1" role="dialog" aria-labelledby="modal-block-fadein" aria-hidden="true">
        <div class="modal-dialog modal-vcenter" role="document">
            <div class="modal-content">
                <span id="form_result"></span>
                <form id="getsentdocument">
                <div class="block block-rounded block-transparent mb-0">
                    <div class="block-header block-header-default">
                    <h3 class="block-title">Kirim Dokumen Investigasi</h3>
                    <input type="text" value="1" name="status_sent_client" id="status_sent_client" hidden>
                    <div class="block-options">
                        <button type="button" class="btn-block-option" data-bs-dismiss="modal" aria-label="Close">
                        <i class="fa fa-fw fa-times"></i>
                        </button>
                    ...
                    <div class="block-content fs-sm text-center">
                        <h5>Apakah yakin akan mengirim dokumen hasil investigasi ke client?</h5>
                    </div>
                    <div class="block-content block-content-full text-end bg-body">
                        <button type="button" class="btn btn-sm btn-alt-secondary me-1" data-bs-dismiss="modal">Cancel</button>
                        <button type="submit" class="btn btn-sm btn-danger btn-send-document">Send Document</button>
                    </div>
                </form>
                </div>
            </div>
    ...
    <!-- END modal edit rekomendasi -->
     ...
     <!-- END modal delete rekomendasi-->
      ...
      <!-- modal rekomendasi baku -->
    <div class="modal fade" id="modal-rekbaku" tabindex="-1" role="dialog" aria-labelledby="modal-block-fadein" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="block block-rounded block-transparent mb-0">
                    <div class="block-header block-header-default">
                        <h3 class="block-title">Rekomendasi Baku</h3>
                        <div class="block-options">
                            <button type="button" class="btn-block-option" data-bs-dismiss="modal" aria-label="Close">
                            <i class="fa fa-fw fa-times"></i>
                            </button>
                    ...
                    <div class="block-content fs-lg" style="align-text:center">
                        <div class="block-content fs-sm">
                            <form id="createrekomndasibaku">
                            <div class="form-floating mb-4">
                                <input hidden type="text" value="{{$detail->id}}" id="investigasi_id" name="investigasi_id">
                                <textarea readonly type="text" class="form-control" id="rekomendasi" name="rekomendasi" rows="10">Bahwa Perusahaan Asuransi harus melakukan pendalaman terhadap agen polis ini, dimana ada indikasi kuat telah mengetahui bahwa Tertanggung menderita sakit seperti temuan diatas namun diindahkan sehingga polis bisa terbit.
                                </textarea>
                                <label for="example-textarea-floating">Rekomendasi Investigasi</label>
                                <input hidden type="text" value="1" id="baku" name="baku">
                            </div>
                        </div>
                    ...
                    <div class="block-content block-content-full text-end bg-body">
                            <button type="button" class="btn btn-sm btn-alt-secondary me-1" data-bs-dismiss="modal">Batal</button> 
                            <button type="submit" class="btn btn-sm btn-danger btn-add-rekomendasi-baku">Aktifkan</button>
                            </form>
                    </div>
